...
cecho "-------IntoRobot W32/W33软件升级--------" $yellow
if [ $sysType = "Darwin" ]; then    # osx
...
$SUDO $ESP32_TOOL --chip esp32 --port $UPLOAD_PORT --baud $UPLOAD_SPEED --before default_reset --after hard_reset write_flash -z --flash_freq $FLASH_SPEED --flash_mode $FLASH_MODE --flash_size $FLASH_SIZE 0x1000 boot-v3.bin 0x8000 partitions.bin 0x9000 blank.bin 0x20000 firmware.bin
